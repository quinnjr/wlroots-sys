...
Raw FFI bindings to [wlroots](https://gitlab.freedesktop.org/wlroots/wlroots) 0.17.
...
**The crate's minor version tracks the wlroots minor version.** `wlr-sys 0.17.x`
binds wlroots 0.17.x, and nothing else. wlroots has no stable ABI and ships a
...
- **Within a minor, the hand-written API is frozen.** Under cargo's 0.x rules
  `0.17.0 → 0.17.1` is an automatic upgrade, so there is no version in which to
...
- **The bound API is whatever your wlroots 0.17.x exposes.** `build.rs` scans the
...
| Requirement | Why |
|---|---|
| wlroots 0.17.x + headers (`wlroots.pc`) | The library being bound |

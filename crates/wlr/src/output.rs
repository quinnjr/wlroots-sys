...
impl<'h> Output<'h> {
    ...
    ///
    /// wlroots 0.15 uses an *implicit* pending state: the setters
    /// (`wlr_output_enable`, `wlr_output_set_mode`, `wlr_output_attach_buffer`
    /// and the rest) stage changes into the output's own `pending` field, and
    /// `wlr_output_commit` applies whatever has accumulated there. Later
    /// wlroots minors replaced that with an explicit `wlr_output_state` value
    /// the caller builds and passes to `wlr_output_commit_state`. The struct
    /// exists here too — it is the type of the output's `pending` field — but
    /// only as wlroots' own storage: there is no `wlr_output_state_init` or
    /// `wlr_output_state_finish` to manage a caller's copy of one, and no
    /// `wlr_output_commit_state` to pass it to. The branches differ
    /// *inside this method* and keep the signature identical, which is what
    /// lets a consumer move between them by changing a version.
    ///
    /// In practice nothing is ever pending here, because this slice exposes
    /// none of the setters that would stage something — enabling an output,
    /// setting a mode, attaching a buffer all arrive with the rendering slice.
    /// So on a disabled output this is observably a no-op, and there is
    /// currently no way to make an output produce a frame.
    ///
    /// # Errors
    ///
    /// [`Error::Operation`] if wlroots rejected the commit.
    pub fn commit(&self) -> Result<()> {
        // SAFETY: the handle's lifetime guarantees the output is live, which is
        // this call's only precondition — the state being committed is the
        // output's own `pending` field, which wlroots initialised when it
        // created the output and owns for its whole life. Nothing here
        // allocates, initialises or frees a state value, because on this
        // version there is none to manage.
        let ok = unsafe { sys::wlr_output_commit(self.raw.as_ptr()) };

        if ok {
            Ok(())
        } else {
            Err(Error::Operation("wlr_output_commit"))
        }
    }
}

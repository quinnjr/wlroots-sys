...
impl<'h> Output<'h> {
    ...
    /// `wlr_output_commit_state`, and 0.17 — this branch — carries both during
    /// the transition. Having the choice, it takes the newer one: it
    /// initialises a fresh `wlr_output_state`, commits that, and finishes it,
    /// which is byte-for-byte what 0.19 and 0.20 do. Using the deprecated
    /// `wlr_output_commit` here would work and would make this branch the odd
    /// one out for no gain, and it would commit whatever happens to be sitting
    /// in `wlr_output.pending` rather than the empty state this documents.
    /// The sibling branches differ *inside this method* and keep the signature
    /// identical, which is what lets a consumer move between them by changing
    /// a version.
    ///

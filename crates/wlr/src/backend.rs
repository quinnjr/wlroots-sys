...
//! 2. The converse also bites. `wlr_backend_autocreate` returns a multi-backend
//!    aggregating whatever the environment offers, and 0.17's header promises
//!    only that `wlr_backend_destroy` is "normally called automatically when the
//!    struct wl_display is destroyed" — *normally*, not solely. Nothing in the
//!    API promises the multi-backend outlives the backends underneath it; later
//!    wlroots releases spell the case out (the primary DRM device being
//!    unplugged destroys the multi-backend), and this branch's headers simply
//!    leave it unstated rather than ruling it out. So the whole `wlr_backend`
//!    can be freed *first*, mid-dispatch, while a `Registration` still holds a
//!    link into it — at which point unlinking is the use-after-free.
//!
//!    This crate cannot settle that question from the outside, and does not
//!    have to: watching `events.destroy` costs one listener and replaces an
//!    unprovable assumption with an observed fact.
//!
...
/// words, "normally called automatically when the struct wl_display is
/// destroyed", and 0.17's `wlr_backend_autocreate` takes that very display and
/// registers the backend with it for exactly that. So `wl_display_destroy` in
/// [`Display`](crate::Display)'s `Drop` is what tears the backend down; calling
...
pub struct Backend<'d> {
    ...
    /// The display whose event loop this backend was created on.
    ///
    ...
    ///
    /// The *display* rather than the loop, unlike the 0.19 and 0.20 branches,
    /// because on 0.17 the display is what `wlr_backend_autocreate` was handed
    /// and so the only pointer this struct has independent reason to hold. The
    /// loop is one `wl_display_get_event_loop` away and is re-derived by
    /// [`EventLoop::from_raw`], which means there is no second stored pointer
    /// that could drift out of agreement with this one.
...
impl<'d> Backend<'d> {
    ...
    pub fn autocreate(loop_: &EventLoop<'d>) -> Result<Self> {
        // 0.17's `wlr_backend_autocreate` takes the `wl_display`, not the
        // `wl_event_loop` 0.19 and 0.20 take. The public signature is the same
        // on every branch, so the difference is absorbed by `EventLoop`
        // carrying its display privately; see that type's own comment.
        //
        // SAFETY: the borrow guarantees the loop — and so the display it was
        // derived from and still names — is live. Null for `session_ptr` only
        // suppresses the out-parameter: wlroots still creates and owns a
        // session when the chosen backend needs one, so the DRM and libinput
        // backends are unaffected and nothing is leaked or left un-torn-down.
        // The only consequence is that this crate holds no session handle,
        // which it has no API to use yet.
        let raw =
            unsafe { sys::wlr_backend_autocreate(loop_.display().as_ptr(), std::ptr::null_mut()) };
    ...
    /// why nothing here asks for a display again: the backend kept the one it
    /// was created from, so there is no second value that could disagree with
    /// it.
    ///
    ...
    pub fn run<S: OutputHandler>(&self, state: &mut S, iterations: u32) -> Result<()> {
        ...
        // SAFETY: `self.display` was taken from the `EventLoop<'d>` handed to
        // `autocreate`, so it names a live display that outlives `'d` — and
        // `&self` keeps that borrow alive for this call. `from_raw` re-derives
        // the loop from it, so the handle built here names the same loop
        // `autocreate` was given rather than a separately remembered one.
        //
        ...
        let loop_ = unsafe { EventLoop::from_raw(self.display) };

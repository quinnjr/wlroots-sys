...
use wlr::{Backend, Display, Handlers, OutputHandler, Runtime};
...
fn a_real_client_creates_a_toplevel_the_server_observes() {
    ...
    let handle = common::client::spawn(&socket, move |state, qh| {
        state.on_created = Some(seen_client);
        state.create_toplevel(qh);
    });

    let mut app = App::default();
    backend.run_all(&mut app, 200).expect("run");
    handle.join().expect("client thread");

    assert!(app.toplevels >= 1, "server should observe the client's toplevel");
}
```
(The `ClientState` fields referenced here are defined in Step 3.)

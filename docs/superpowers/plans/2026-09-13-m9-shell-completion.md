...
- **Bench-leg waiver (`Surface` dispatch):** no `crates/wlr/benches/surfaces.rs` was added — `with_surface` and the dispatcher are crate-private and only drivable through an active `Backend::run_all`, whose per-event cost measures the session and event loop rather than the safe layer (the `signal_emit` probe in `benches/dispatch.rs` documents the same bar), so generic-surface delivery stays covered by the positive-path tests in `tests/surfaces.rs` instead. Revisit if `with_surface` becomes public or a synchronous dispatch-drive hook lands.
